...
from find_intent import IntentFinder, intent_questions
...
if __name__ == "__main__":
    ...
    intentchecker = IntentFinder(int(device.split(':')[-1]), ckpt='./ckpt/qa_b.ckpt')
    transer = Trans(int(device.split(':')[-1]), ckpt='./ckpt/tr_b.ckpt')
    ...
    if args.interactive_mode:
        ...
        for index, context in enumerate(
            ...
            for _ in range(5):
                ...
                intentRes = intentchecker.classify_intent([' '.join(dialog[-2:])])
                print('151', intentRes)
                ...
                if(intentRes != 'No'):
                    int_appear = True
                    rep = transer.gen_trans(simulator, simulator_tokenizer, 
                                            dialog[max(len(dialog) - 5, 0):], 
                                                context, intentRes)
                    dialog[-1] = rep
                
                if not (int_appear) and _ >= 2:
                    intentRes = np.random.choice(list(intent_questions.keys()))
                    rep = transer.gen_trans(simulator, simulator_tokenizer, 
                                            dialog[max(len(dialog) - 5, 0):], 
                                                context, intentRes)
                    dialog[-1] = rep
                if not args.disable_output_dialog:
                    print(f"\033[0;33;49m {'bot: ': ^11}{dialog[-1]} \033[0;0m")
