...
class Encoder(nn.Module):
    def __init__(self, embeddings, hidden_size, batch_size=16, n_layers=1, device='cuda:0', input_size=None):
        ...
        self.lstm = nn.LSTM(embeddings.shape[-1], hidden_size, n_layers, batch_first=True,bidirectional=True)
    ...
    def init_hidden(self,input):
        hidden = Variable(torch.zeros(self.n_layers*2, input.size(0), self.hidden_size)).to(self.device)
        context = Variable(torch.zeros(self.n_layers*2, input.size(0), self.hidden_size)).to(self.device)
