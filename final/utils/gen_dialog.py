...
if __name__ == "__main__":
    ...
    # load your bot
    bot = (BlenderbotForConditionalGeneration if args.model_name_or_path[-3:] == 'ill' else AutoModelForCausalLM).from_pretrained(args.model_name_or_path).to(device)
    bot_tokenizer = BlenderbotTokenizer.from_pretrained(args.model_name_or_path)
    DS = 'daily_dialog'
    dataset = load_dataset(DS, split=args.split)
    dataset = dataset.map(
        preprocess if DS=='blended_skill_talk' else dprep,
        remove_columns=[
            "free_messages",
            "guided_messages",
            "suggestions",
            "personas",
            "additional_context",
        ] if DS=='blended_skill_talk' else ['act', 'emotion'],
    )
    ...
    if args.interactive_mode:
        ...
        for index, context in enumerate(
            ...
            for _ in range(5):
                # print(f'130 cont: {context}, diag:{dialog}')
                inputs = simulator_tokenizer(
                    [
                        "</s> <s>".join(
                ...
                inputs = bot_tokenizer(
                    ["</s> <s>".join(dialog[max(0, len(dialog)-3):])], return_tensors="pt", truncation=True
                ).to(device)
                reply_ids = bot.generate(**inputs)
                text = bot_tokenizer.batch_decode(reply_ids, skip_special_tokens=True)[0].strip()
                # print('149', dialog[max(0, len(dialog)-3):], bot_tokenizer.batch_decode(chat_history_ids))
                dialog.append(text)
                if not args.disable_output_dialog:
                    print(f"\033[0;33;49m {'bot: ': ^11}{text} \033[0;0m")

            output.append((context.split('\n') if DS != 'blended_skill_talk' else '') + dialog)
            if not args.disable_output_dialog:
                print()
